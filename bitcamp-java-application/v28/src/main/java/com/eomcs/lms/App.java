...
public class App {
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (command.length() == 0)
        continue;
      commandStack.push(command); // 사용자가 입력한 명령을 보관한다. (입력한 값 거꾸로 출력)
      commandQueue.offer(command); // 사용자가 입력한 명령을 보관한다. (입력한 값 차례로 출력)
 ...
 // File의 정보를 준비 
    ...
    try {
      ...
      for (Member member : memberList) {
        ...
        // => 수업 데이터를 한 문자열로 만들자.
        //   형식은 국제적으로 많이 사용하는 CSV(Comma-Separated Value) 형식으로 만들자.
        String str = String.format("%d,%s,%s,%s,%s,%s\n", 
            member.getNo(), 
            member.getName(), 
            member.getPassword(), 
            member.getEmail(), 
 ...
 // File의 정보를 준비 
    ...
    try {
      ...
      while (scan.hasNextLine()) {
        ...
        // 수업 데이터를 담을 Board 객체를 준비한다. 
        Board board = new Board();

        // 배열 각 항목에 값을 Board 객체에 담는다.
        board.setNo(Integer.parseInt(data[0]));
        board.setContents(data[1]);
        board.setCreatedDate(Date.valueOf(data[2]));
        
        // 수업 데이터를 담은 Board 객체를 BoardList에 추가한다.
        boardList.add(board);
      }
